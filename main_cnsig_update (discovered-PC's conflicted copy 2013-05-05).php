...
<script type="text/javascript">
	...
	$(document).ready(function()
	{
		$('table#multipels td a.delete').click(function()
		{
			...
			var tablename="main_consignee";
			var subtable1="tender_firm_product";
			var subcolmn1="consignee";
				...
				$.ajax(
				{
					   ...
					   success: function(data)
					   {
							...
							parent.fadeOut('slow', function() {$(this).remove();});
							   ...
							   {
								   alert("Record can't be deleted \n Please check tender's Consinies.");
							   }
						}
...
function EditItsUrl(pid)
{
	if(pid!="")
	{
	window.location="main_cnsig_update.php?pid="+pid;
	}
...
<div id="index-main">
<span id="title_work_page">Update/Delete Firm</span>
<div id="working-sub-main">
<div id="working-panel">
<table style="margin-left:13px; margin-top:10px;">
<tr>
<td style="font-size:12px;">Select Purchaser</td>
...
<?php foreach($AllPurchaser as $d) 
{
    ...
    <option value="<?=$d['id'];?>" <?=${'_P_'.$d['id']};?>><?=$d['purchaser_short_name'];?></option>
    <?php
}
?>
...
<tr><th width="200">Name</th><th width="200">Created by</th><th width="200">Updated by</th><th width="70">Edit</th><th width="70">Delete</th>
<th width="70">View</th>
</tr>
<?php 
foreach($result as $v)
...
<td style="text-align:center"  class="basic_text_style"><label><font style="font-size:12px"><?=$v['main_csign_short_name']; ?></font></label></td>
<td style="text-align:center" class="basic_text_style"><label><font style="font-size:12px"><?php if($v['created_by_name']=="") { echo "-";} else { ?><a class="opener" onclick="AjaxMsg('<?=$v['main_csign_createdby'];?>','<?=$v['created_table'];?>')" style="cursor:pointer;"><?=$v['created_by_name']; } ?></a></font></label></td>
<td style="text-align:center" class="basic_text_style"><label><font style="font-size:12px"><?php if($v['updated_by_name']=="") { echo "-";} else { ?><a class="opener" onclick="AjaxMsg('<?=$v['main_csign_updatedby'];?>','<?=$v['updated_table'];?>')" style="cursor:pointer;"><?=$v['updated_by_name']; } ?></a></font></label></td>
<td width="100" style="text-align:center"><a href="main_cnsig_registration.php?val=<?=$v['id'];?>&pid=<?=$_GET['pid'];?>"><img src="main_images/edit.png" /></td></a>
<td width="100" style="text-align:center"><a href="#" id="<?=$v['id'];?>" class="delete"><img src="main_images/delete.png" /></a></td>
<td width="100" style="text-align:center"><a href="All_profile_view_files/Main_consignee_Profile.php?val=<?=$v['id'];?>&pid=<?=$_GET['pid'];?>" target="_blank"><img src="main_images/view.png" /></a></td>
</tr>
