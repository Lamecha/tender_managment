...
if(isset($_POST['save']))
{
	...
	if($_FILES['file']['size']!=0)
	{
		...
		move_uploaded_file($tmp_name,"attachements/tender_attach_history/".$finalName);
	}
	else
	{
		$finalName=0;
	...
	$result = $obj->Update_Tender_Attachements_History($_POST['titles'],$_POST['other_title'],$finalName,$name,$_POST['id_firm_attachement']);
	if($result==1)
	{
		?>
        <script>
       window.location="history_tender_old_attacements_record.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>";
		</script>
        <?php
...
function Edit_Directors(ss)
{
	...
	window.location="history_tender_old_attacements_record.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&tb_row="+ss;
}
</script>
<script>
...
<option value="4" <?=$_4_M;?>>Annulav</option>
<option value="5" <?=$_5_M;?>>Special Caution</option>
