...
if(isset($_POST['save']))
{
	...
	if($_FILES['file']['size']!=0)
	{
		...
		move_uploaded_file($tmp_name,"attachements/officer_attach/".$finalName);
	}
	else
	{
		$finalName=0;
	...
	$result = $obj->Update_Officer_Attachements($_POST['titles'],$_POST['other_title'],$finalName,$name,$_POST['id_firm_attachement']);
	if($result==1)
	{
		?>
        <script>
       window.location="Officer_Old_Attachements.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>";
		</script>
        <?php
...
function Edit_Directors(ss)
{
	...
	window.location="Officer_Old_Attachements.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&tb_row="+ss;
}
</script>
<script>
