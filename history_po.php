...
if(isset($_GET['val']) && $_GET['val']!="")
{
	...
	$result_product = $obj->common_fetch_attachement_withItem_PO_HISTORY('history_po_firm_product','po_id',$_GET['val']);
	$result_attachment = $obj->common_fetch_attachement('history_po_attachements','po_id',$_GET['val']);
...
if(isset($_POST['submit']) && $_POST['key']==$_SESSION['key'])
{
		...
		for($i=0;$i<sizeof($_POST['titles']);$i++)
		{
			...
			move_uploaded_file($tmp_name,"attachements/po_attach_history/".$finalName[$i]);
		}
	...
	if(isset($_GET['val']) & $_GET['val']!="")
		...
		$update_result = $obj->Po_Update_History_Section($_POST,$finalName,$name,$_SESSION['pin_id'],$_SESSION['pin_table'],$_GET['val']);
			if($update_result == 1)
			{
			$obj->ALertMessage("Po [History] updated",$update_result);
			$obj->redirect("edit_history_po.php");
			}
		}
		else
		{
	...
	$tender = $obj->po_registration_history_wala($_POST,$finalName,$name,$_SESSION['pin_id'],$_SESSION['pin_table']);
	$obj->ALertMessage("Po [History] registered",$tender);
...
<title><?php if(isset($_GET['val'])){echo "Update";}else{"Fill";} ?> PO History Section</title>
...
function getAllInspection(INS)
{
	...
	if(INS!=6)
	qData += "<option value='5'>RITES/Visual</option>"
	if(INS!=7)
	qData += "<option value='5'>RITES/MTC & GC</option>"
	if(INS!=8)
	qData += "<option value='5'>DQA/Visual</option>"
	if(INS!=9)
	qData += "<option value='5'>DQA/MTC & GC</option>"
	if(INS!=10)
...
function getAllTaxType(IU)
{
	if(IU==1)
	var option='VAT Exclusive';
	if(IU==2)
	var option='VAT Inclusive';
	if(IU==3)
	var option='CST Exclusive';
	if(IU==4)
	var option='CST Inclusive';
	if(IU==5)
	var option='Nil';
	...
	if(IU!=1)
	uData += "<option value='1'>VAT Exclusive</option>"
	if(IU!=2)
	uData += "<option value='2'>VAT Inclusive</option>"
	if(IU!=3)
	uData += "<option value='3'>CST Exclusive</option>"
	if(IU!=4)
	uData += "<option value='4'>CST Inclusive</option>"
	if(IU!=5)
	uData += "<option value='5'>Nil</option>"
	return uData;
...
function getSelectBoxTaxType(value,id,IU)
{
	...
	new_select_unit.setAttribute("name",value);
	new_select_unit.setAttribute("class","selectD");
	new_select_unit.setAttribute("style","width:150px;");
	new_select_unit.setAttribute("id",'tax_type'+id);

	
	var new_opt_unit = getAllTaxType(IU);
//	new_opt.innerHTML="helllo";
	new_select_unit.innerHTML=new_opt_unit;
	return new_select_unit;
...
<div id="index-main">
<span id="title_work_page"><?php if(isset($_GET['val'])){echo "Update";}else{"Fill";} ?> PO History</span>
<div id="working-sub-main">
<div id="working-panel" style="width:1500px;">
...
<tr>
<td><label for="tender_office">PO Office<font color="#990000">*</font></label></td>
<td><select id="tender_office" name="po_office" class="validate[required] Xtext-input">
...
<td><select id="tender_purchaser" name="tender_purchaser" class="validate[required] Xtext-input" onChange="CnsignListonPurchaserHistory(this.value);">
<?php if(isset($_GET['val'])) { ?>
...
<tr>
<?php 
$selected_firm = '_'.$tender_firm['id'].'_FIRM';
$$selected_firm = "selected='selected'";
 ?>
<td><label for="tender_office">Firms<font color="#990000">*</font></label></td>
<td><select id="po_firms" name="po_firms" class="validate[required] Xtext-input" >
<?php if(!isset($_GET['val'])) { ?>
<option value="">Select</option>
<?php } ?>
<?php foreach($list_firm as $listfirmski)

{?>
<option value="<?=$listfirmski['id'];?>" <?=${'_'.$listfirmski['id'].'_FIRM'};?>><?=$listfirmski['csign_short_name'];?></option>
<?php
}?>
</select></td></tr>
...
<tr><td><label for="tender_number">PO No.<font color="#990000">*</font></label></td>
<td><input type="text" name="po_number" id="<?php if(isset($_GET['val']) && $_GET['val']!="") { echo $_GET['val'] ; } else
{ echo "po_number"; } ?>"  class="validate[required,ajax[ajaxPoCallPhp]] text-input" class="XAutoFillTender" value="<?php print_r($result['po_number']); ?>" onBlur="XCheckTenderNumber(this.value),XAddReTenderInfo(this.value)" onKeyUp="XCheckTenderNumber(this.value)"/></td><td><span id="tender_number_span"><img id="loader_no" style="display:none;" src="main_images/indicator.gif" /></span></td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>
<tr><td><label for="po_due_date">PO Date<font color="#990000">*</font></label></td>
<td><input type="text" name="po_due_date" id="datepicker-example1_nsci" value="<?php echo $final_date; ?>" style="width:120px;" class="validate[required] Xtext-input"/></td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>
<tr><td><label for="po_due_date">PO Value<font color="#990000">*</font></label></td>
<td><input type="text" name="po_value" id="po_value" value="<?php print_r($result['po_value']); ?>" class="validate[required] Xtext-input"/></td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>
...
<caption>Products<?php
   ...
   { 
?>
/<a class="link_edit_director" onClick="window.open('history_po_products_old.php?val=<?php print_r($result['id']); ?>&table=history_po_firm_product&field=po_id&purchaser='+document.getElementById('tender_purchaser').value,'mywindow','width=1500,height=333,left=100,top=100,screenX=0,screenY=100')">Edit old products</a>
<?php
}
?></caption>
<tr id='base_id'><th style="width:200px;">Item Category</th><th>Item Inspection</th><th style="width:200px;">Item Discription</th><th>Consignee</th><th>Quantity</th><th>Unit</th><th style="width:150px;">Tax Type</th><th style="width:150px;">Tax</th><th style="width:150px;">Rate</th></tr>
<?php 
if(is_array($result_product))
{
    ...
    <td><input type="text" name="tax_type_update[]" id="tax_type_update_0" style="width:150px;" class="selectD" value="<?php echo $my_taxtype;  ?>" readonly/></td>
    <td><input type="text" name="tax_update[]" id="tax_update_0" style="width:150px;" class="selectD" value="<?=$mj['tax'];?>" readonly/></td>
    <td><input type="text" name="rate_update[]" id="rate_update_0" style="width:150px;" class="selectD" value="<?=$mj['rate'];?>" readonly/></td>
    </tr>
	<?php
	}
}
...
<td><select name="tax_type[]" id="tax_type0" style="width:150px;" class="selectD"/>
<option value="">Please select</option>
<option value='1'>VAT Exclusive</option>
<option value='2'>VAT Inclusive</option>
<option value='3'>CST Exclusive</option>
<option value='4'>CST Inclusive</option>
<option value='5'>Nil</option>
</select></td>
<td><input type="text" name="tax[]" id="tax0" style="width:150px;"/></td>
<td><input type="text" name="rate[]" id="rate0" style="width:150px;"/></td>
</tr>
...
<input type="button" name="add_more" id="add_more" value="Add More Product" class="add_more" onClick="add_more_products_History_Po();"/>
</td>
</tr>
</table>
...
<caption>Attachments <?php if($result_attachment[0]!='') { ?> <a class="link_edit_director" onClick="window.open('Po_old_attachemnents.php?val=<?php echo $_GET['val']; ?>&table=history_po_attachements&field=po_id','mywindow','width=500,height=300,left=100,top=100,screenX=0,screenY=100')">/Delete old </a><?php } ?></caption>
<tr id="base_id_attachements"><th>Title</th><th>File</th></tr>
<?php if(isset($_GET['val']) && $_GET['val']!='') { ?>
<?php foreach($result_attachment as $FFF) { ?>
<?php
$title_value = $obj->Title_In_PO_History_Attachements($FFF['title']);
?>
<tr>
...
<option value="">Please Select</option>
<option value="1">IC Copy</option>
<option value="2">PO Copy</option>
<option value="3">Challan</option>
<option value="4">Bill Copy</option>
<option value="5">Full Case</option>
<option value="6">R. Note</option>
<option value="10">Others</option>
</select>
</td>
...
<tr style="background:#fff;"><td>&nbsp;</td><td><input type="button" name="add_more" id="add_more" value="Add More" class="add_more" onClick="Add_More_Po_Attachements();"/></td></tr>
