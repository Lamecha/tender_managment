...
if(isset($_POST['save']))
{
	...
	if($_FILES['file']['size']!=0)
	{
		...
		move_uploaded_file($tmp_name,"attachements/specification/".$finalName);
	}
	else
	{
		$finalName=0;
	...
	$result = $obj->Update_Specification_Attachements($finalName,$name,$_POST['id_firm_attachement']);
	if($result==1)
	{
		?>
        <script>
       window.location="specification_old_attachements.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>";
		</script>
        <?php
...
function Edit_Directors(ss)
{
	...
	window.location="specification_old_attachements.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&tb_row="+ss;
}
</script>
<script>
...
-->
<td style="text-align:center; font-size:12px;color:#FFF; height:30px;"><?=$v['file'];?></td>
