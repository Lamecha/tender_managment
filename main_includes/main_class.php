...
public function specification_registration($specification_no,$part,$revision,$reaffirmed,$year,$finalName,$name,$created_by,$created_table,$issued,$other_issued)
	{
		...
		$updated_by=NULL;
		$updated_table=NULL;
		$query = $this->conn->prepare("INSERT INTO `specification`(`specification_no`,`part`,`revision`,`reaffirmed`,`year`,`issued`,`other_issued`,`created_by`,`created_table`,`updated_by`,`updated_table`) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
$query->bind_param("sssssssssss",$specification_no,$part,$revision,$reaffirmed,
$year,$issued,$oIs,$created_by,$created_table,$updated_by,$updated_table);
		$query->execute();
		$query->store_result();
...
public function Specification_update($specification_no,$part,$revision,$reaffirmed,$year,$finalName,$name,$id,$updated_by,$updated_table,$issued,$other_issued)
{
	if(trim($other_issued)=='')
		{
			$oIs = NULL;
	...
	$query = $this->conn->query("UPDATE `specification` SET `specification_no` = '".$specification_no."', `part` = '".$part."',`revision` = '".$revision."',`reaffirmed` = '".$reaffirmed."', `year` = '".$year."',`issued`='".$issued."',`other_issued`='".$oIs."',`updated_by` = '".$updated_by."',`updated_table`='".$updated_table."' WHERE `id` LIKE '".$id."'  ");
	if($query->error)
		{
			return 0;
		}
...
public function ChangeTenderFirmStatusXXX($id,$status)
{
...
public function ChangeCommonStatus($id,$status,$table)
{
	$query = $this->conn->query("UPDATE `".$table."` SET `status`='".$status."' WHERE `id` LIKE ".$id);
	if($query)
	{
		return 1;
	}
...
$query_update = $this->conn->query("SELECT * FROM `tender_rate_attachements` WHERE  `tender_id` LIKE '".$tender_id."'");
	...
	{
		$query = $this->conn->query("SELECT tender_firms.*,consignee.csign_short_name FROM `tender_firms`,`consignee` WHERE tender_firms.firm LIKE consignee.id AND tender_firms.product_id LIKE ".$product_id);
		while($raw = $query->fetch_assoc())
		{
			$data[]=$raw;
		}
	...
	{
		$query = $this->conn->query("SELECT tender_firms.bid_number,tender_firms.remark,consignee.csign_short_name FROM `consignee`,`tender`,`tender_firms`,`tender_firm_product` WHERE tender_firms.firm LIKE consignee.id AND tender_firms.product_id LIKE tender_firm_product.id AND tender_firm_product.tender_id LIKE tender.id AND tender.id LIKE '".$tender_id."' ORDER BY consignee.csign_short_name ASC");
		while($raw = $query->fetch_assoc())
		{
			$data[] = $raw;
		}
