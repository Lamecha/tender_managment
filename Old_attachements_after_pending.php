...
if(isset($_POST['save']))
{
	...
	if($_FILES['file']['size']!=0)
	{
		...
		move_uploaded_file($tmp_name,"attachements/attachements_after_pending_status/".$finalName);
	}
	else
	{
		$finalName=0;
	...
	$result = $obj->Update_Pending_Attachements($finalName,$name,$_POST['id_firm_attachement']);
	if($result==1)
	{
		
		?>
        <script>
       window.location="Old_attachements_after_pending.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>";
		</script>
        <?php
...
{
	window.location="Old_attachements_after_pending.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&tb_row="+ss;
}
</script>
<script>
function RemoveReadOnly()
{
	...
	if(a==10)
	{
		...
		document.getElementById('other_title').setAttribute('class','validate[required] Xtext-input')
...
<option value="1" <?//=$_1_M;?>>BIS</option>
<option value="2" <?//=$_2_M;?>>DGQA</option>
<option value="3" <?//=$_3_M;?>>Railway</option>
<option value="4" <?//=$_4_M;?>>DGS & D</option>
...
{	
	//$title_value = $obj->Title_In_Specification($v['title']);
?>
<tr>
<!--
...
-->
<td style="text-align:center; font-size:12px;color:#FFF; height:30px;"><a href="attachements/attachements_after_pending_status/<?=$v['file_real_name'];?>" target="_blank"><?=$v['file'];?></a></td>
