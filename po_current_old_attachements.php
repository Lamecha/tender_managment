...
if(isset($_POST['save']))
{
	...
	if($_FILES['file']['size']!=0)
	{
		...
		move_uploaded_file($tmp_name,"attachements/po_final_attachements/".$finalName);
	}
	else
	{
		$finalName=0;
	...
	$result = $obj->Update_PO_CURRENT_OLD_Attachements($_POST['titles'],$_POST['other_title'],$finalName,$name,$_POST['id_firm_attachement']);
	if($result==1)
	{
		?>
        <script>
       window.location="po_current_old_attachements.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>";
		</script>
        <?php
...
<title>Old PO Attachments : Rainbow Tender Managment</title>
...
function Edit_Directors(ss)
{
	...
	window.location="po_current_old_attachements.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&tb_row="+ss;
}
</script>
<script>
...
<option value="1" <?=$_1_M;?>>NIT</option>
<option value="2" <?=$_2_M;?>>Specification</option>
<option value="3" <?=$_3_M;?>>Drawing Copy</option>
<option value="4" <?=$_4_M;?>>Annexures</option>
<option value="5" <?=$_5_M;?>>Special Conditions</option>
...
$title_value = $obj->Title_In_Tender_Attachements($v['title']);
?>
<tr>
