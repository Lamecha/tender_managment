...
if(isset($_GET['val']) && $_GET['val']!="")
{
	...
	$result = $obj->common_fetch_attachement_withItem_nolimit_his_wl($_GET['table'],$_GET['field'],$_GET['val']);
...
<script type="text/javascript">
	...
	$(document).ready(function()
	{
		$('table#multipels td a.delete').click(function()
		{
			...
			var tablename="history_tender_firm_product";
...
{
	window.location="record_product_history_tender.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&purchaser=<?=$_GET['purchaser'];?>&km=km&tb_row="+ss;
...
function Edit_Directors_2()
{
	...
	var Update_products_In_tender_History = 12;
	...
	var main_consignee_contactsCall;  
			...
			main_consignee_contactsCall.onreadystatechange = function()
			{
				...
				{
				window.location="record_product_history_tender.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&purchaser=<?=$_GET['purchaser'];?>";
				window.opener.document.getElementById("update_row"+pidProductFirm).innerHTML=main_consignee_contactsCall.responseText;
				}
			}
	
	main_consignee_contactsCall.open("GET","MainAjax_Part2.php?category="+category+"&inspection="+inspection+"&discription="+discription+"&consignee="+consignee+"&quantity="+quantity+"&unitX="+unitX+"&pidProductFirm="+pidProductFirm+"& Update_products_In_tender_History="+ Update_products_In_tender_History,true);
