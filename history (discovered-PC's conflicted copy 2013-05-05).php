...
<title><?php if(isset($_GET['val'])){echo "Update";}else{"Fill";} ?> Tender History Section</title>
...
function getAllInspection(INS)
{
	...
	if(INS=='')
	{
		qData = "<option value=''>Please Select</option>"
	}
	else
	{
...
<input type="button" id="remove_old" value="Remove Old" onClick="javascript:DeleteAllOldssInHistory();" style="position:absolute; left:900px; top:10px; z-index:100;"/>
...
<td><input type="text" name="tender_number" id="presidentsServerInput" class=<?php if(!isset($_GET['val'])){echo "AutoFillTender_In_History";}else { echo "NoAutoFillTender_In_History"; } ?> value="<?php print_r($result['tender_number']); ?>" onKeyUp="CheckTenderNumber_inHistory_Tender(this.value)"/></td><td><span id="tender_number_span"><img id="loader_no" style="display:none;" src="main_images/indicator.gif" /></span></td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>
...
<caption>Products<?php
   ...
   { 
?>
/<a class="link_edit_director" onClick="window.open('record_product_history_tender.php?val=<?php print_r($result['id']); ?>&table=history_tender_firm_product&field=tender_id&purchaser='+document.getElementById('tender_purchaser').value,'mywindow','width=1020,height=333,left=100,top=100,screenX=0,screenY=100')">Edit old products</a>
<?php
}
?></caption>
...
<caption>Attachments <?php if(isset($_GET['val']) && $result_attachment[0]!='') { ?> <a class="link_edit_director" onClick="window.open('history_tender_old_attacements_record.php?val=<?php print_r($result['id']); ?>&table=history_tender_attachments&field=tender_id','mywindow','width=500,height=300,left=100,top=100,screenX=0,screenY=100')">/Delete old </a><?php } ?></caption>
...
<option value="4">Annulav</option>
<option value="5">Special Caution</option>
<option value="10">Others</option>
</select>
</td>
