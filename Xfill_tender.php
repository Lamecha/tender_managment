...
<link rel="stylesheet" href="css/jqautocomplete.css"/>
<script type="text/javascript" src="js/autocomplete.jquery.js"></script>
<script type="text/javascript">

var $tl=jQuery.noConflict();
//window.pID=$tl('#tender_purchaser').val();
function ItsMe(pID)
{
	
   var json_options = {
   script:'test.php?json=true&idz='+pID+'&',
        varname:'input',
		json:true,
        shownoresults:true,
		maxresults:16,
		callback: function (obj) { $tl('#json_info').html('you have selected: '+obj.id + ' ' + obj.value + ' (' + obj.info + ')'); }
		};
var xml_options = {
		...
		shownoresults:true,
		maxresults:16,
		callback: function (obj) { $tl('#xml_info').html('you have selected: '+obj.id + ' ' + obj.value + ' (' + obj.info + ')'); }
		};
		
		$tl(function(){
			//$tl('#test_json').focus(function() {
				
				
		$tl('#test_json').autoComplete(json_options);
		$tl('#test_xml').autoComplete(xml_options);
		//});
		});
}
</script>
...
<td><input type="text" name="tender_number" id="test_json" value="<?php print_r($result['tender_number']); ?>" /></td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr>
