...
if(isset($_GET['val']) && $_GET['val']!="")
{
	...
	$result = $obj->common_fetch_attachement_withItem_nolimit_Po_History($_GET['table'],$_GET['field'],$_GET['val']);
...
{
	window.location="history_po_products_old.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&purchaser=<?=$_GET['purchaser'];?>&km=km&tb_row="+ss;
...
function Edit_Directors_2()
{
	...
	var tax_type = document.getElementById('tax_type_update_0_X').value;
	var tax = document.getElementById('tax_update_0_X').value;
	var rate = document.getElementById('rate_update_0_X').value;
	var meenu=312;
	...
	var main_consignee_contactsCall;  
			...
			main_consignee_contactsCall.onreadystatechange = function()
			{
				...
				{
				window.location="history_po_products_old.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&purchaser=<?=$_GET['purchaser'];?>";
				window.opener.document.getElementById("update_row"+pidProductFirm).innerHTML=main_consignee_contactsCall.responseText;
				}
			}
	
	main_consignee_contactsCall.open("GET","MainAjax_Part2.php?category="+category+"&inspection="+inspection+"&discription="+discription+"&consignee="+consignee+"&quantity="+quantity+"&unitX="+unitX+"&pidProductFirm="+pidProductFirm+"&tax_type="+tax_type+"&tax="+tax+"&rate="+rate+"&Update_Products_in_Po_History="+meenu,true);
...
//	}
	?>
    -->
<tr><th style="width:200px;">Item Category</th><th style="width:150px;">Item Inspection</th><th style="width:150px;">Item Discription</th><th style="width:150px;">Consignee</th><th style="width:130px;">Quantity</th><th style="width:140px;">Unit</th><th style="width:140px;">Tax Type</th><th style="width:140px;">Tax</th><th style="width:140px;">Rate</th><th style="width:80px;">Edit</th><th style="width:80px;">Delete</th>
</tr>
<?php
foreach($result as $v)
{
	...
	$my_taxtype = $obj->taxType_in_Po($v['tax_type']);
	if($v['id']==$_GET['tb_row'])
	{
...
<?php }  ?>
	...
	<option value='6' <?=$_6_U;?>>Yard</option>
     ...
     <td>
    <?php
	$Tax_opt='_'.$v['tax_type'].'_Tax';
	$$Tax_opt="selected='selected'"; 
	?>
    <select  name="tax_type_update_X[]" id="tax_type_update_0_X" style="width:120px;" class="selectD"/>
	<option value='1' <?=$_1_Tax;?>>VAT Exclusive</option>
	<option value='2' <?=$_2_Tax;?>>VAT Inclusive</option>
	<option value='3' <?=$_3_Tax;?>>CST Exclusive</option>
	<option value='4' <?=$_4_Tax;?>>CST Inclusive</option>
	<option value='5' <?=$_5_Tax;?>>Nil</option>
	</select>
    </td>
     <td><input type="text" name="tax_update_X[]" id="tax_update_0_X" style="width:120px;background:#FFF;" value="<?=$v['tax'];?>"/></td>
      <td><input type="text" name="rate_update_X[]" id="rate_update_0_X" style="width:120px;background:#FFF;" value="<?=$v['rate'];?>"/></td>
...
<td><input type="text" name="category_update[]" style="width:200px; background:#0066ff;" class="kareena" value="<?=$v['item_name'].":-".$v['item_discription'].':('.$v['item_id'].')';?>" id="category_update_0" readonly />
    ...
    <td><input type="text" name="tax_type_update[]" id="tax_type_update_0" style="width:120px;background:#0066ff;" readonly value="<?=$my_taxtype;?>"/></td>
    <td><input type="text" name="tax_update[]" id="tax_update_0" style="width:120px;background:#0066ff;" readonly value="<?=$v['tax'];?>"/></td>
    <td><input type="text" name="rate_update[]" id="rate_update_0" style="width:120px;background:#0066ff;" readonly value="<?=$v['rate'];?>"/></td>
