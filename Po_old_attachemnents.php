...
if(isset($_POST['save']))
{
	...
	if($result==1)
	{
		?>
        <script>
       window.location="Po_old_attachemnents.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>";
		</script>
        <?php
...
function Edit_Directors(ss)
{
	...
	window.location="Po_old_attachemnents.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&tb_row="+ss;
}
</script>
<script>
...
<option value="1" <?=$_1_M;?>>IC Copy</option>
<option value="2" <?=$_2_M;?>>PO Copy</option>
<option value="3" <?=$_3_M;?>>Challan</option>
<option value="4" <?=$_4_M;?>>Bill Copy</option>
<option value="5" <?=$_5_M;?>>Full Case</option>
<option value="6" <?=$_6_M;?>>R. Note</option>
...
$title_value = $obj->Title_In_PO_History_Attachements($v['title']);
?>
<tr>
