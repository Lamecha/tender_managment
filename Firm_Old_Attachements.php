...
if(isset($_POST['save']))
{
	...
	if($_FILES['file']['size']!=0)
	{
		...
		move_uploaded_file($tmp_name,"attachements/csign_attach/".$finalName);
	}
	else
	{
		echo 'else';
	...
	$result = $obj->Update_Firm_Attachements($_POST['titles'],$_POST['other_title'],$finalName,$name,$_POST['id_firm_attachement']);
	if($result==199)
	{
		?>
        <script>
        window.location="Firm_Old_Attachements.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>";
		</script>
        <?php
...
<title>3ttachments : Rainbow Tender Managment</title>
...
function Edit_Directors(ss)
{
	...
	window.location="Firm_Old_Attachements.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&tb_row="+ss;
}
</script>
<script>
...
?>
   <form id="muform" name="myform" action="#" enctype="multipart/form-data" method="post">
	...
	<option value="1" <?=$_1_M;?>>NSIC Reg</option>
	<option value="2" <?=$_2_M;?>>DGS & D Reg</option>
	<option value="3" <?=$_3_M;?>>DGQA Reg</option>
	<option value="4" <?=$_4_M;?>>CST/VAT Reg</option>
	<option value="5" <?=$_5_M;?>>Performance</option>
	<option value="6" <?=$_6_M;?>>Digital Signature</option>
	<option value="7" <?=$_7_M;?>>PAN Card</option>
	<option value="8" <?=$_8_M;?>>Certificate at Incorporation</option>
	<option value="9" <?=$_9_M;?>>Memorandum at Article</option>
...
<td style="text-align:center; font-size:12px"><input type="file" id="file" name="file">
...
$title_value = $obj->Title_In_Firm_Registration($v['title']);
?>
<tr>
