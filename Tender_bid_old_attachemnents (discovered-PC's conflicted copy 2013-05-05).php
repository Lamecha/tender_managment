...
if(isset($_POST['save']))
{
	...
	if($_FILES['file']['size']!=0)
	{
		...
		move_uploaded_file($tmp_name,"attachements/tender_bid_attach/".$finalName);
	}
	else
	{
		$finalName=0;
	...
	$result = $obj-> Update_Tender_Attachements_bid_wale($_POST['titles'],$_POST['other_title'],$finalName,$name,$_POST['id_firm_attachement']);
	if($result==1)
	{
		?>
        <script>
       window.location="Tender_bid_old_attachemnents.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>";
		</script>
        <?php
...
function Edit_Directors(ss)
{
	...
	window.location="Tender_bid_old_attachemnents.php?val=<?=$_GET['val'];?>&table=<?=$_GET['table'];?>&field=<?=$_GET['field'];?>&tb_row="+ss;
}
</script>
<script>
...
<option value="1" <?=$_1_M;?>>Financial Tabulation</option>
<option value="2" <?=$_2_M;?>>Technical Tabulation</option>
<option value="3" <?=$_3_M;?>>Mannual Tabulation</option>

<option value="4" <?=$_4_M;?>>Correspondance</option>
</select>
    </td>
...
$title_value = $obj->Title_In_Tender_Bid_Attachements($v['title']);
?>
<tr>
